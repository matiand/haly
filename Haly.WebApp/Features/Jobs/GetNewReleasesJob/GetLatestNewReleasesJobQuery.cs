...
public class GetLatestNewReleasesJobHandler(LibraryContext db)
    : IRequestHandler<GetLatestNewReleasesJobQuery, NewReleasesJobDto?>
{
    public async Task<NewReleasesJobDto?> Handle(GetLatestNewReleasesJobQuery request,
        CancellationToken cancellationToken)
    {
        var latestCompletedJob = await db.CollectNewReleasesJobs
