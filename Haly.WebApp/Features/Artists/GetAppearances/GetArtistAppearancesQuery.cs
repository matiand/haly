...
public class GetArtistAppearancesHandler(ISpotifyService spotify)
    : IRequestHandler<GetArtistAppearancesQuery, ArtistAppearancesDto>
{
    public async Task<ArtistAppearancesDto> Handle(GetArtistAppearancesQuery request,
        CancellationToken cancellationToken)
    {
        var releases =
            await spotify.GetArtistReleases(request.Id, ArtistRelease.AppearsOn, request.UserMarket);
